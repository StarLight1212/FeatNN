...
#     # UPDATE
#     add_idx = np.array([i*(n_nbs*n_vertex) for i in range(0,(ebd_size)*batch_size,ebd_size)])
#     add_idx = np.transpose(add_idx.reshape(-1,batch_size))
#     add_idx = add_idx.reshape(-1)
#     # TODO: Concatenate processing?
#     # new_array = input_array.reshape(-1)+add_idx
#     # new_array = np.concatenate((input_array.reshape(-1), add_idx))
#     new_array = input_array.reshape((-1))
...
def split_train_test_clusters(measure, clu_thre, n_fold):
    # load cluster dict
    cluster_path = '../preprocessing/All/'
    ...
    # c_kf = KFold(len(C_cluster_list), shuffle=True)
    # p_kf = KFold(len(P_cluster_list), shuffle=True)
...
def load_data(measure, setting, clu_thre, n_fold):
    # load data
    with open('../preprocessing/All/pdbbind_all_combined_input_'+measure,'rb') as f:
    ...
    if setting == 'imputation':
        ...
        for fold in range(n_fold):
            ...
            train_idx, valid_idx, test_idx = [], [], []
            # 将cluster_index映射回真实的标签，如果有重复cluster_index则根据其在train，valid还是test中给出索引ele作为最终索引
            for ele in range(n_sample):
...
def loading_emb(measure):
    ...
    f = open('../preprocessing/All/pdbbind_all_atom_dict_'+measure, 'rb')
    atom_dict = pickle.load(f)
    f.close()

    f = open('../preprocessing/All/pdbbind_all_bond_dict_'+measure, 'rb')
    bond_dict = pickle.load(f)
    f.close()

    f = open('../preprocessing/All/pdbbind_all_word_dict_'+measure, 'rb')

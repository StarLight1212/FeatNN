# This script is used for extract and calculate the sequence information and structure information
# (DDM: discrete distance matrix and TorM: torsion matrix) for the PDB file
...
def get_pdbid_lst(path):
    for root, diag, file in os.walk(path):
        print(file)
    file_out = [i[:-4] for i in file]
    return file_out


# def get_pdbid_list_from_txt(file_path):
#     pdbid_list = []
...
def get_res_coord(structure):
    ...
    for chain in model:
        ...
        seq_dict[chain.get_id().upper()] = seq
...
def calc_dist_matrix(distance_matrix_dict, min_distance=3.25, max_distance=50.75, step=1.25):
    ...
    for chain, coord in distance_matrix_dict.items():
        ...
        for row, res1 in enumerate(coord):
            ...
            # print(euclidean_distance)
        distance_dict[chain.upper()] = distance_matrix
        # print(distance_dict)
    ...
    for model in structure:
        for chain in model:
            ...
            if len(phi_torsion) != len(seq):
                ...
                return None, None
            phi_torsion_dict[chain.get_id().upper()] = phi_torsion[:]
            psi_torsion_dict[chain.get_id().upper()] = psi_torsion[:]
            phi_torsion.clear()
            psi_torsion.clear()
...
def output_distance_matrix_and_dihedral_angle(pdb_id):
    try:
        structure = Bio.PDB.PDBParser().get_structure(pdb_id, './pdb_files/' + pdb_id + '.pdb')
...
if __name__ == '__main__':
    ...
    i = 0
    valid_pdb_list = []
    pdbid_list = get_pdbid_lst('./pdb_files/')
    # pdbid_list = get_pdbid_list_from_txt('./pdbid.txt')
    pdbid_to_ligand = get_pdbid_to_ligand('./fileprocessing/pdbbind_index/INDEX_general_PL.2020')

    output_dist_mtx_folder = r"./DDM_TM/"
    # Automatically create new folder
    ...
    for pdbid in pdbid_list[:]:
        ...
        print(output_dict)
        np.save(
            open(f"{output_dist_mtx_folder}/{pdbid}_{ligand}.npy", 'wb'),
            output_dict
        )
    print("Length of valid pdbid is: ", len(valid_pdb_list))
    np.save(output_dist_mtx_folder+'valid_pdbid.npy', valid_pdb_list)

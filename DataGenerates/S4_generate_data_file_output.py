...
def get_fasta_dict():
    uniprot_dict = {}
    name, seq = '', ''
    with open('./fileprocessing/output/out1.6_pdbbind_seqs.fasta') as f:
    ...
    with open('./fileprocessing/output/out1.6_uniprot_uniprot_mapping.tab') as f:
...
def get_pdbid_to_uniprotid(seq_dict):
    ...
    with open(
            './fileprocessing/pdbbind_index/INDEX_general_PL_name.2020') as f:
        for line in f.readlines():
            if line[0] != '#':
    ...
    with open('./fileprocessing/output/out1.4_pdb_uniprot_mapping.tab') as f:
        for line in f.readlines()[1:]:
            lines = line.split('\t')
    ...
    for pdbid in pdbid_set:
        if pdbid in uniprot_mapping_dict and len(uniprot_mapping_dict[pdbid]) == 1:
            ...
            if pdbid not in pdbbind_mapping_dict:
                count += 1
                continue
...
# write file
count_success = 0
with open(r'./fileprocessing/output/out2_pdbbind_data_datafile.tsv', 'w') as fw:
    ...
    for pdbid in pdbid_to_uniprotid:
        ...
        fw.write(pdbid + '\t' + uniprotid + '\t' + ligand + '\t' + inchi + '\t' + seq + '\t' + measure + '\t' + str(
            value) + '\n')
        count_success += 1
fw.close()
